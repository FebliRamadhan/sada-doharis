...
const AUTH_CODE_EXPIRES = parseInt(process.env['OAUTH_AUTHORIZATION_CODE_EXPIRES_IN'] ?? '600', 10);
...
export const oauthService = {
  ...
  async exchangeAuthorizationCode(data: {
    ...
    // Generate tokens
    const accessToken = tokenService.generateAccessToken(authCode.userId, 'user', authCode.scopes);
   ...
   */
  async clientCredentialsGrant(data: { clientId: string; clientSecret: string; scopes: string[] }) {
    // Validate client using bcrypt comparison
    const client = await clientService.validateCredentials(data.clientId, data.clientSecret);
    if (!client) {
      throw new InvalidClientError('Invalid client credentials');
    ...
    const accessToken = tokenService.generateAccessToken(client.id, 'client', requestedScopes);
   ...
   * Refresh token grant
   */
  async refreshTokenGrant(data: { refreshToken: string; clientId: string; clientSecret: string }) {
    // Validate client using bcrypt comparison
    const client = await clientService.validateCredentials(data.clientId, data.clientSecret);
    if (!client) {
      throw new InvalidClientError('Invalid client credentials');
    ...
    const accessToken = tokenService.generateAccessToken(subjectId, type, storedToken.scopes);
