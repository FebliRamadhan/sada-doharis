  ...
  describe('authenticate', () => {
    it('should authenticate user successfully', async () => {
      ...
      mockSearch.mockImplementation(
        (_base: string, _opts: unknown, cb: (err: Error | null, res: EventEmitter) => void) => {
          cb(null, searchRes);
    ...
    it('should reject when service account bind fails', async () => {
      ...
      await expect(ldapService.authenticate('john.doe', 'password123')).rejects.toThrow(
    ...
    it('should reject when user is not found in LDAP', async () => {
      ...
      await expect(ldapService.authenticate('unknown.user', 'password123')).rejects.toThrow(
        'User not found'
      );
    ...
    it('should reject when user password is incorrect', async () => {
      ...
      await expect(ldapService.authenticate('john.doe', 'wrong_password')).rejects.toThrow(
        'Invalid credentials'
      );
