...
describe('oauthService', () => {
  ...
  describe('generateAuthorizationCode', () => {
    ...
    it('drops `internal`/`government` scopes for non-matching user types', async () => {
      ...
      (prisma.user.findUnique as ReturnType<typeof vi.fn>).mockReturnValueOnce({
        userType: 'GOVERNMENT',
      });
      ...
      const createArg = (prisma.oAuthAuthorizationCode.create as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
