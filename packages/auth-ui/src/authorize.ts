...
function buildAuthorizeUrl(oauthParams: OAuthParams, withConsent = false): string {
  ...
  if (oauthParams.codeChallengeMethod)
    url.searchParams.set('code_challenge_method', oauthParams.codeChallengeMethod);
  if (oauthParams.nonce) url.searchParams.set('nonce', oauthParams.nonce);
...
async function init(): Promise<void> {
  ...
  try {
    ...
    if (!isValidRedirect) {
      showError(
        'Invalid redirect URI. This application is not authorized to use this redirect URI.'
      );
      return;
    }
