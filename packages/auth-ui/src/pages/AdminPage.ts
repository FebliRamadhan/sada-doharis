...
const S = {
  ...
  badge:
    'display:inline-flex;align-items:center;gap:0.375rem;padding:0.25rem 0.625rem;border-radius:9999px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em',
  label:
    'display:block;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#64748B;margin-bottom:0.5rem',
  input:
    'width:100%;padding:0.75rem 1rem;border-radius:0.75rem;font-size:0.875rem;font-weight:500;background:#F0F1F3;color:#191C1D;border:none;outline:2px solid transparent',
} as const;
...
function renderClientRow(c: OAuthClient): string {
  const initials = (
    c.name.charAt(0) + (c.name.split(/\s/)[1]?.charAt(0) || c.name.charAt(1) || '')
  ).toUpperCase();
  ...
  const scopePills =
    c.scopes
      ...
      .map(
        (s) =>
          `<span style="display:inline-block;padding:0.125rem 0.5rem;border-radius:0.25rem;font-size:10px;font-weight:600;background:rgba(1,52,124,0.05);color:#354E6B">${s}</span>`
      )
      .join(' ') +
    (c.scopes.length > 3
...
function attachClientsEvents(): void {
  ...
  paginationListeners('clients', async (p) => {
    await loadClients(p);
    renderMain();
  });
}
      ...
      <div style="display:flex;align-items:center;gap:0.75rem;flex-shrink:0">
        <select id="logs-filter" style="${S.input};width:auto;padding:0.625rem 1rem;cursor:pointer">
          ...
          ${[
            ...
            .map(
              (a) =>
                `<option value="${a}" ${state.logsActionFilter === a ? 'selected' : ''}>${a}</option>`
            )
            .join('')}
        </select>
...
function attachLogsEvents(): void {
  ...
  paginationListeners('logs', async (p) => {
    await loadLogs(p);
    renderMain();
  });
}
...
function renderPagination(
  ...
  const pageBtnBase =
    'padding:0.25rem 0.75rem;font-size:0.75rem;font-weight:700;border-radius:0.25rem;border:none;cursor:pointer';
  ...
  const navBtn = (disabled: boolean) =>
    `padding:0.25rem;border-radius:0.25rem;border:none;background:transparent;cursor:${disabled ? 'default' : 'pointer'};color:#64748B;opacity:${disabled ? '0.3' : '1'}`;
...
function openCreateModal(): void {
  ...
  document.getElementById('modal-submit')?.addEventListener('click', async () => {
    ...
    const uris = (document.getElementById('f-uris') as HTMLTextAreaElement).value
      ...
      .filter(Boolean);
    const grants = [
      ...document.querySelectorAll<HTMLInputElement>('input[name="grant"]:checked'),
    ].map((e) => e.value);
    const scopes = [
      ...document.querySelectorAll<HTMLInputElement>('input[name="scope"]:checked'),
    ].map((e) => e.value);
...
function openDeleteConfirm(id: string, name: string): void {
  ...
  document.getElementById('modal-submit')?.addEventListener('click', async () => {
    ...
    await loadClients(
      Math.max(1, state.clients.length === 1 ? state.clientsMeta.page - 1 : state.clientsMeta.page)
    );
    renderMain();
  });
}
...
function openRegenConfirm(id: string, name: string): void {
  ...
  document.getElementById('modal-submit')?.addEventListener('click', async () => {
    setSubmitLoading(true);
    const r = await apiRequest<OAuthClient>(`${endpoints.clients}/${id}/regenerate-secret`, {
      method: 'POST',
    });
    setSubmitLoading(false);
    if (!r.success) {
...
const iconMonitor = (s = 16) =>
  svg(`<rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/>`, s);
const iconLog = (s = 16) =>
  svg(
    `<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>`,
    s
  );
const iconLogout = (s = 14) =>
  svg(
...
const iconTrash = (s = 14) =>
  svg(
    `<polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/>`,
    s
  );
const iconRefresh = (s = 14) =>
  svg(
    `<polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/>`,
    s
  );
const iconPlus = (s = 16) =>
  svg(`<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>`, s);
const iconCopy = (s = 14) =>
  svg(
